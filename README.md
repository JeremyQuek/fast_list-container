...
Inspired by HFT order book implementations, I wanted to recreated a basic version of it to test the real performance of hyper optimised C++. This list supporst an average of up 25 million operations / second.
<div align="center">
...
5) ID Map. Uses an array of boolean fails, each array indexes uses pointer and memroy arithmetic to obtain the ith list node after the buffer start memory location
...
7) Uses integer for iding rather pointers. Squeezes even less bytes into a node struct. 
